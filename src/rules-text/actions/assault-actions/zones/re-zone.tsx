...
export const reZone: Item = {
  ...
  element:
    <>
      ...
      <h3>Расположение:</h3>
        <div>C каждой стороны от главного входа напротив укрелений, на расстоянии примерно около 15 метров от главного входа</div>

      <h3>Игровая условность:</h3>
        <div>На фазе подготовки штурма интендант атакующей стороны оставляет на каждом из вымпелов определеное количество лент воскрешения исходя из общего числа очков подкреплений</div>
        <div>Если боец атакующей сторны умер, он может воскреснуть возле зоны подкрепления</div>
        <div>Зона подкрепления не считается закрытой зоной и игрока могут убить сразу после воскрешения</div>
        <div>Если сторона обороны смогла захватить вымпел из зоны подкрепления, отнеся его к себе в закрытую зону, то считается что данная зона подкреплени вместе с лентами выходит из боя</div>
